    ...
    <>
      ...
      <header className="relative bg-gradient-to-b from-pasir via-[#efe5cf] to-[#e8d6ad] px-5 pt-24 pb-14 md:grid md:min-h-[92svh] md:content-center md:py-16">
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl text-center md:text-left">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3 md:justify-start">
            <Link href="/destinasi" className="rounded-full bg-kombu px-7 py-3 text-sm font-bold text-pasir hover:bg-[#b04636] transition">
              Jelajahi Destinasi
            </Link>
            <Link href="/blog/itinerary-sumba-5-hari" className="rounded-full border-2 border-indigo2 px-7 py-3 text-sm font-bold text-indigo2 hover:bg-indigo2/5 transition">
              Lihat Itinerary
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {destinasi.map((d, i) => (
            <FadeIn key={d.slug} delay={i * 80} direction="up">
              <Link
                href={`/destinasi/${d.slug}`}
                className="group block overflow-hidden rounded-2xl border border-indigo2/15 bg-white transition hover:-translate-y-1 hover:shadow-lg"
              >
      ...
      <FadeIn className="mx-5 mb-16 md:mx-auto md:max-w-6xl">
        <div id="kontak" className="rounded-3xl bg-gradient-to-br from-kombu to-[#6e2419] p-10 text-center text-pasir">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="mt-6 inline-block rounded-full bg-[#1f8a4c] px-8 py-3.5 font-bold text-white hover:bg-[#27a25b] transition"
          >
            Chat via WhatsApp
          </a>
        </div>
