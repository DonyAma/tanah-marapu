    ...
    <section className="mx-auto max-w-6xl px-5 py-14">
      ...
      {q && (
        <p className="mt-4 text-sm text-[#8a8273]">
          Hasil pencarian: <b className="text-kombu">"{searchParams.q}"</b>{" "}
          <Link href="/destinasi" className="underline">hapus</Link>
        </p>
      )}

      {data.length === 0 ? (
        <FadeIn className="mt-10 rounded-2xl border border-indigo2/15 bg-white p-10 text-center">
          <p className="font-display text-lg font-bold text-indigo2">Tidak ditemukan</p>
          <p className="mt-1 text-sm text-[#5a5347]">Coba kata kunci lain, atau jelajahi semua destinasi.</p>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {data.map((d, i) => (
            <FadeIn key={d.slug} delay={i * 70} direction="up">
              <Link
              ...
              >
                <div className={`relative h-44 overflow-hidden bg-gradient-to-br ${d.gradien}`}>
                  {d.foto && (
                    <ImageWithSkeleton
                      src={d.foto}
                      alt={d.nama}
                  ...
                  <span className={`absolute top-3 right-3 z-10 rounded-full px-3 py-1 text-[10px] font-extrabold uppercase tracking-wider ${
